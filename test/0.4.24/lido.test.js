...
contract('Lido', ([appManager, , , , , , , , , , , , user1, user2, user3, nobody, depositor, treasury]) => {
  ...
  const setupNodeOperatorsForELRewardsVaultTests = async (userAddress, initialDepositAmount) => {
    ...
    const amounts = []
    for (let i = 0; i < initialDepositAmount / ETH(32); ++i) {
      amounts.push(ETH(32))
    }
    await app.methods[`deposit(uint256,uint256,bytes,uint256[])`](MAX_DEPOSITS, curated.id, CALLDATA, 
      amounts, { from: depositor })
  }
  ...
  context('EL Rewards', async () => {
    ...
    it('Attempt to set invalid execution layer rewards withdrawal limit', async () => {
      ...
      const setupNodeOperatorsForELRewardsVaultTests = async (userAddress, initialDepositAmount) => {
        ...
        await operators.addSigningKeys(0, 1, pad('0x010203', 48), pad('0x01', 96), 
          [ethers.Wallet.createRandom().address], { from: voting })
        await operators.addSigningKeys(
          0,
          3,
          hexConcat(pad('0x010204', 48), pad('0x010205', 48), pad('0x010206', 48)),
  ...
  it('Lido.deposit(uint256,uint256,bytes,uint256[]) reverts when called by account without DEPOSIT_ROLE granted', async () => {
    await assert.reverts(
      app.methods['deposit(uint256,uint256,bytes,uint256[])'](MAX_DEPOSITS, CURATED_MODULE_ID, CALLDATA, [ETH(32)], 
        { from: nobody }),
      'APP_AUTH_DSM_FAILED'
    )
  })
  ...
  it('deposit works', async () => {
    ...
    await assert.reverts(
      app.deposit(MAX_DEPOSITS, CURATED_MODULE_ID, CALLDATA, [ETH(0)], { from: depositor }),
    ...
    await assert.revertsWithCustomError(
      app.methods['deposit(uint256,uint256,bytes,uint256[])'](MAX_DEPOSITS, CURATED_MODULE_ID, CALLDATA, 
    ...
    await app.deposit(1, 1, CALLDATA, [ETH(32)], { from: depositor })
  ...
  it('deposit uses the expected signing keys', async () => {
    ...
    const tos0 = Array.from({ length: 3 }, (_) => ethers.Wallet.createRandom().address);
    const op1 = {
      keys: Array.from({ length: 3 }, (_, i) => `0x22${i}${i}` + 'efab'.repeat(46 / 2)),
      sigs: Array.from({ length: 3 }, (_, i) => `0x22${i}${i}` + 'fcde'.repeat(94 / 2))
    }

    const tos1 = Array.from({ length: 3 }, (_) => ethers.Wallet.createRandom().address);
    await stakingRouter.setWithdrawalCredentials(pad('0x0202', 32), { from: voting })

    await operators.addSigningKeys(0, 3, hexConcat(...op0.keys), hexConcat(...op0.sigs), tos0, { from: voting })
    await operators.addSigningKeys(1, 3, hexConcat(...op1.keys), hexConcat(...op1.sigs), tos1, { from: voting })
  ...
  it('deposit works when the first node operator is inactive', async () => {
    ...
    await operators.addSigningKeys(0, 1, pad('0x010203', 48), pad('0x01', 96), [ethers.Wallet.createRandom().address], 
      { from: voting })
    await operators.addSigningKeys(1, 1, pad('0x030405', 48), pad('0x06', 96), [ethers.Wallet.createRandom().address],
      { from: voting })
  ...
  it('handleOracleReport works', async () => {
    ...
    await assert.reverts(
      app.handleOracleReport(await getCurrentBlockTimestamp(), 1, ETH(30), [ETH(32)], 0, 0, 0, 0, [], 0, { from: appManager }),
      'APP_AUTH_FAILED'
    )
    ...
    await assert.reverts(
      app.handleOracleReport(await getCurrentBlockTimestamp(), 1, ETH(29), [ETH(32)], 0, 0, 0, 0, [], 0, { from: nobody }),
      'APP_AUTH_FAILED'
    )
  ...
  it('oracle data affects deposits', async () => {
    ...
    await assert.reverts(app.methods['deposit(uint256,uint256,bytes,uint256[])'](MAX_DEPOSITS, CURATED_MODULE_ID, CALLDATA, 
      [ETH(32)], { from: depositor }), "deposits more then buffered Ether")
